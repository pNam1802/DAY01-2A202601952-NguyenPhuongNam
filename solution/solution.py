...
def call_openai(
    ...
    """
    from openai import OpenAI  # import BÊN TRONG hàm để mock hoạt động
...
def compare_models(prompt: str) -> dict:
    ...
    # Ước lượng thô chi phí output của GPT-4o: 0.75 từ ≈ 1 token
    gpt4o_cost = (
...
def count_tokens(text: str, model: str = OPENAI_MODEL) -> int:
    ...
    try:
        ...
        # Model lạ / offline → ước lượng dự phòng: 1 token ≈ 4 ký tự
...
def estimate_cost(prompt: str, response: str, model: str = OPENAI_MODEL) -> dict:
    ...
    # Model không có trong bảng giá (vd model NIM miễn phí) → lấy giá gpt-4o
    pricing = PRICING_PER_1K_TOKENS.get(model, PRICING_PER_1K_TOKENS["gpt-4o"])
    input_cost = input_tokens / 1000 * pricing["input"]
...
def retry_with_backoff(
    ...
    """
    for attempt in range(max_retries + 1):  # lần đầu + max_retries lần thử lại
        try:
            return fn()
        except Exception:
            if attempt == max_retries:
                raise  # hết lượt → ném nguyên exception gốc ra
...
def run_assistant(
    ...
    while True:
        # Kiểm tra max_turns TRƯỚC khi đọc input (max_turns=0 thoát ngay)
        if max_turns is not None and num_turns >= max_turns:
            break
        ...
        # System prompt (persona) LUÔN đứng đầu → không bị trôi mất khi cắt history
        messages = (
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    header = f"{'Prompt':<40} | {'GPT-4o':<40} | {'Mini':<40} | {'4o (s)':>7} | {'Mini (s)':>8}"
    lines = [header, "-" * len(header)]
    for r in results:
        lines.append(
            f"{clip(r['prompt']):<40} | {clip(r['gpt4o_response']):<40} | "
            f"{clip(r['mini_response']):<40} | {r['gpt4o_latency']:>7.2f} | "
            f"{r['mini_latency']:>8.2f}"
        )
    return "\n".join(lines)
